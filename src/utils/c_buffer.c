...
** Last update Wed Apr  8 23:06:36 2015 Jean-Baptiste Grégoire
*/
...
void		cbuffer__write(t_cbuffer *cbuf, char *str, size_t size)
{
  ...
  while (size > 0)
	...
	to_write = rest;
      ...
      pos += rest;
      size -= to_write;
      cbuf->write_pos = (cbuf->write_pos + to_write) % BUF_SIZE;
...
char		*cbuffer__read(t_cbuffer *cbuf, size_t size)
{
  ...
  while (size > 0)
	...
	return (NULL);
      ...
      size -= to_read;
      cbuf->read_pos = (cbuf->read_pos + to_read) % BUF_SIZE;
      free(buf);
    }
  return (ret);
}
